...
pub fn textDocument_documentColor(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) callconv(.c) i32 {
    ...
    const hints_arr = switch (hints_val) {
        .array => |arr| arr.items,
        else => return -1,
    };
...
pub fn textDocument_colorPresentation(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) callconv(.c) i32 {
    ...
    const params = std.json.parseFromValue(
        lsp.types.ColorPresentation.Params,
        a,
        params_val,
        .{ .ignore_unknown_fields = true },
    ) catch return -1;
...
pub fn textDocument_inlayHint(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) callconv(.c) i32 {
    ...
    const hints_val = obj.get("hints") orelse return -1;
    const hints = switch (hints_val) {
        .array => |arr| arr.items,
        else => return -1,
    };
    const docParams_val = obj.get("docParams") orelse return -1;
    const docParams = switch (docParams_val) {
        .array => |arr| arr.items,
        else => return -1,
    };
    ...
    for (hints) |hint_val| {
        const h = switch (hint_val) {
            .object => |o| o,
            else => continue,
        };
        const text = switch (h.get("text") orelse continue) {
            .string => |s| s,
            else => continue,
        };
