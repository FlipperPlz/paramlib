...
pub const ArrayIdentifier = identifiers.TypedId("Array", .arr, *ArrayData, *const ArrayData);
...
pub const ArrayInit = struct {
    pub const _identifier = ArrayIdentifier;
...
pub fn ArrayStorage(comptime field: []const u8) type {
    return struct {
        ...
        pub fn next(self: Self, store: *const storage.ParamAllocator) !Self {
            ...
            const data: *ArrayData = try store.retrieve(self.handle.id);
