...
pub fn createClass(allocator: Allocator, io: std.Io, store: *storage.ParamAllocator, init: class.ClassInit) !*const class.ClassData {
    const clazz = try store.alloc(allocator, io, .clazz, .createClass(init));
...
pub fn getOrCreateClass(allocator: Allocator, io: std.Io, store: *storage.ParamStorage, init: class.ClassInit) !*class.ClassData {
    ...
    if (resolved_init.pathHash == null) {
        const path = if (init.parent) |parentHandle| blk: {
            ...
            break :blk hasher.IncrementalHasher.load(parentData.pathHash).updateSep().update(init.name).final();
        } else blk: {
...
pub fn deleteClass(
    ...
    while (child.hasNext()) {
        ...
        const childData: *class.ClassData = (try childHandle.validateHandle(store)).ptr;
    ...
    while (p.hasNext()) {
        ...
        const paramData: *params.ParameterData =  (try paramHandle.validateHandle(store)).ptr;
        p = paramData.sibling;
