...
pub const ClassData = packed struct {
    ...
    pub fn init(
        ...
        const timestamp = time.getTimeMs(io);
        const name = try store.intern(allocator, paths.getName(args.path));
        const name_hash = hasher.hash(name.ptr);
        const name_idx = name.id;
