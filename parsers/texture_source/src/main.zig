...
const ProcTextureToken = enum {
    ...
    Co, Ca, No, Ns, Dt, Mc, Sm, Smdi,
};

const TOKENS = std.StaticStringMap(ProcTextureToken).initComptime(.{
    .{ ",",                .Comma           },
    .{ "(",                .OpenParen       },
    .{ ")",                .CloseParen      },
    .{ "#",                .Hash            },
            ...
            .Dither => blk: {
                ...
                break :blk switch(proc_id) {
                    ...
                    .Text => .{ .Text = TextArgsDiag{
                        .args = .{
                            .vAlign = .Center, .hAlign = .Center, .fontName = "", .fontSize = 0,
                            .backgroundColor = .{ .r = 0, .g = 0, .b = 0, .a = 0 },
                            .textColor = .{ .r = 0, .g = 0, .b = 0, .a = 0 },
                            .text = "",
                        },
                        .vAlign_off = 0, .hAlign_off = 0, .fontName_off = 0, .fontSize_off = 0,
                        .backgroundColor_off = 0, .textColor_off = 0, .text_off = 0,
                    } },
                    else => .Point,
                };
            }
...
const IrradianceArgs = struct { const Diag = IrradianceArgsDiag; specularPower: i32 };
...
const DitherArgs = struct { const Diag = DitherArgsDiag; min: i32, max: i32 };
...
const PerlinNoiseArgs = struct { const Diag = PerlinNoiseArgsDiag; xScale: i32, yScale: i32, min: i32, max: i32 };
...
const TreeCrownArgs = struct { const Diag = TreeCrownArgsDiag; density: f32 };
const TreeCrownArgsDiag = struct {
    args: TreeCrownArgs,
    density_off: u32,
};

const FresnelArgs = struct { const Diag = FresnelArgsDiag; refractiveIndex: f32, absorptionCoefficient: i32 };
...
const Render2TextureArgs = struct { const Diag = Render2TextureArgsDiag; surface: []const u8, aspect: f32 };
...
fn parseProcTexture(src: []const u8) ?ProcTexture {
    ...
    defer temp_diags.deinit(alloc);
    const dpt = parseProcTextureWithDiags(alloc, src, &temp_diags, .{.line=0, .character=0, .text="", .length=0}, .{.base_char=0, .quoted=false});
...
fn resolvePos(offset: u32, line_offsets: []const u32) lsp.types.Position {
    ...
    while (lo < hi) {
        ...
        if (line_offsets[mid] < offset) { lo = mid + 1; } else { hi = mid; }
    }
...
export fn textDocument_completion(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) i32 {
    ...
    const root       = std.json.parseFromSlice(std.json.Value, a, in_ptr[0..in_len], .{}) catch return -1;
    const obj        = switch (root.value) { .object => |o| o, else => return -1 };
    const params_obj = switch (obj.get("params") orelse return -1) { .object => |o| o, else => return -1 };
    const pos_obj    = switch (params_obj.get("position") orelse return -1) { .object => |o| o, else => return -1 };
    ...
    const hints_arr    = getHintsArr(obj) orelse return -1;
    const lo_arr       = switch (obj.get("lineOffsets") orelse return -1) { .array => |a_| a_.items, else => return -1 };
    ...
    for (hints_arr) |hint_val| {
        ...
        if (in_proc) {
            for ([_][]const u8{
                ...
                "point", "fresnel", "r2t", "text", "ui", "uiex", "extension",
            }) |p|
                items.append(a, .{ .label = p, .kind = .Function, .detail = "procedural texture" }) catch continue;
        }
    }

    return jsonWrite(a, lsp.types.completion.Result{ .completion_items = items.items },
        .{ .emit_null_optional_fields = false }, out_ptr, out_max);
}
...
export fn textDocument_colorPresentation(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) i32 {
    ...
    const label = a.dupe(u8, std.fmt.bufPrint(&label_buf, "{d:.4},{d:.4},{d:.4},{d:.4}",
...
export fn textDocument_diagnostic(in_ptr: [*]const u8, in_len: usize, out_ptr: [*]u8, out_max: usize) i32 {
    ...
    const obj = switch (root.value) {
        .object => |o| o,
        else    => { log("textDocument_diagnostic: root is not an object", .{}); return -1; },
    };
    ...
    for (hints_arr) |hint_val| {
        ...
        const dpt = ctx.parseDiag(a, &diags) orelse {
            if (diags.items.len == 0) diags.append(a, .{
                .range    = .{ .start = .{ .line = ctx.hint.line, .character = ctx.hint.character },
                               .end   = .{ .line = ctx.hint.line, .character = ctx.hint.character + ctx.hint.length } },
                .severity = .Error,
        ...
        if (pt.width > 0 and pt.height > 0) {
            if (!isPowerOfTwo(pt.width)) diags.append(a, .{
                .range    = .{ .start = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.width_off },
                               .end   = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.width_off  + @as(u32, @intCast(std.mem.indexOfScalar(u8, ctx.src[dpt.width_off..],  ',') orelse 0)) } },
                .severity = .Error,
                .message  = "Texture width must be a power of 2.",
            }) catch {};
            if (!isPowerOfTwo(pt.height)) diags.append(a, .{
                .range    = .{ .start = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.height_off },
                               .end   = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.height_off + @as(u32, @intCast(std.mem.indexOfScalar(u8, ctx.src[dpt.height_off..], ',') orelse 0)) } },
                .severity = .Error,
            ...
            if (pt.nMipmaps > 0 and (@as(u31, 1) << @intCast(pt.nMipmaps - 1)) > max_dim) diags.append(a, .{
                .range    = .{ .start = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.mips_off },
                               .end   = .{ .line = ctx.hint.line, .character = ctx.hp.base_char + dpt.mips_off  + @as(u32, @intCast(std.mem.indexOfScalar(u8, ctx.src[dpt.mips_off..],  ')') orelse 0)) } },
                .severity = .Error,
